...
c = n[h.col], e = o[h.col], c = c < e ? -1 : c > e ? 1 : 0, 0 !== c) return "desc" === h.dir ? c : -c; c = d[a]; e = d[b]; return c < e ? -1 : c > e ? 1 : 0
    ...
    for (var b, c, d = a.aoColumns, e = Q(a), a = a.oLanguage.oAria, f = 0, i = d.length; f < i; f++) {
        ...
        var h = g(c.nTh).removeAttr("aria-sort"); c.bSortable && (0 < e.length && e[0].col == f ? (h.attr("aria-sort", "desc" == e[0].dir ? "ascending" : "descending"), c = j[e[0].index + 1] || j[0]) : c = j[0], b += "desc" === c ? a.sSortAscending : a.sSortDescending); h.attr("aria-label", b)
    } 
} function Db(a, b, c, d) {
...
oClasses:{},oLanguage:{oAria:{sSortAscending:": انقر لترتيب العمود تنازلياً",sSortDescending:": انقر لترتيب العمود تصاعدياً"},oPaginate:{sFirst:">",sLast:"<",sNext:">",sPrevious:"<"},sEmptyTable:"لا توجد بيانات فى الجدول.",sInfo:"عـرض _START_ إلى _END_ من _TOTAL_ سـجل",sInfoEmpty:"عـرض 0 إلى 0 من 0 سـجل",sInfoFiltered:"(نتيجة البحث من أصل _MAX_ سـجل)",sInfoPostFix:"",sInfoThousands:",",sLengthMenu:"عـرض _MENU_ سـجل",sLoadingRecords:"جارى التجميل...",sProcessing:"جارى التنفيذ...",
    sSearch: "بـحث:", sUrl: "", sZeroRecords: "لا توجد بيانات مطابقة للبحث."
...
}; T(o.defaults); o.defaults.column = { aDataSort: null, iDataSort: -1, asSorting: ["desc", "desc"], bSearchable: !0, bSortable: !0, bVisible: !0, fnCreatedCell: null, mData: null, mRender: null, sCellType: "td", sClass: "", sContentPadding: "", sDefaultContent: null,
...
}, function (a) { return ca(a) || "string" === typeof a && -1 !== a.indexOf("<") ? "html" : null } ]); g.extend(o.ext.type.search, { html: function (a) { return ca(a) ? "" : "string" === typeof a ? a.replace(Hb, " ").replace(va, "") : "" }, string: function (a) { return ca(a) ? "" : "string" === typeof a ? a.replace(Hb, " ") : a } }); g.extend(!0, o.ext.renderer, { header: { _: function (a, b, c, d, e) {
    g(a.nTable).on("order.dt", function (a, g, j, h) {
        b.removeClass(c.sSortingClass + " " + e.sSortAsc + " " + e.sSortDesc).addClass(h[d] == "desc" ? e.sSortAsc : h[d] ==
"desc" ? e.sSortDesc : c.sSortingClass)
    ...
    g("<div/>").addClass(e.sSortJUIWrapper).append(b.contents()).append(g("<span/>").addClass(e.sSortIcon + " " + c.sSortingClassJUI)).appendTo(b); g(a.nTable).on("order.dt", function (a, g, j, h) {
        b.removeClass(e.sSortAsc + " " + e.sSortDesc).addClass(h[d] == "desc" ? e.sSortAsc : h[d] == "desc" ? e.sSortDesc : c.sSortingClass); b.find("span").removeClass(e.sSortJUIAsc + " " + e.sSortJUIDesc + " " + e.sSortJUI + " " + e.sSortJUIAscAllowed + " " + e.sSortJUIDescAllowed).addClass(h[d] ==
"desc" ? e.sSortJUIAsc : h[d] == "desc" ? e.sSortJUIDesc : c.sSortingClassJUI)
    })
} 
}
